...
import { generateScript, generateTasks, refineNotes } from '../services/geminiService';
...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, currentUserRole, channels, onClose, onUpdate }) => {
  ...
  const handleGenerateScript = async () => {
    ...
    const script = await generateScript(localProject.title, localProject.platform, localProject.vertical);
    const updated = { ...localProject, script, lastUpdated: Date.now() };
    setLocalProject(updated);
    onUpdate(updated);
  ...
  const handleGenerateTasks = async () => {
    setIsGenerating(true);
    const newTasks = await generateTasks(localProject.stage, localProject.platform);
    const updated = { 
        ...localProject, 
        tasks: [...localProject.tasks, ...newTasks],
        lastUpdated: Date.now()
    };
    setLocalProject(updated);
    onUpdate(updated);
  ...
  const handleRefineNotes = async () => {
     setIsGenerating(true);
     const refined = await refineNotes(localProject.technicalNotes);
     const updated = { ...localProject, technicalNotes: refined, lastUpdated: Date.now() };
     setLocalProject(updated);
     onUpdate(updated);
     setIsGenerating(false);
  };
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                 <div className="flex justify-between items-center">
                    <label className="text-xs font-mono text-[#666] uppercase">AI Script Editor</label>
                    <button 
                      onClick={handleGenerateScript}
                      disabled={isGenerating}
                      className="flex items-center space-x-2 px-3 py-1 bg-indigo-600/20 text-indigo-400 hover:bg-indigo-600/30 text-xs rounded-full transition-colors disabled:opacity-50">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <button 
                      ...
                      <Sparkles size={12} /> <span>Suggest Tasks</span>
                    </button>
                  </div>
                  <div className="space-y-2">
               ...
               <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <button 
                        ...
                        disabled={isGenerating}
                        className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center space-x-1">
                        <Sparkles size={12} /> <span>Refine with AI</span>
                    </button>
                  </div>
                  <textarea 
